...
if (!class_exists('USP_Pro_Process')) {
	...
	class USP_Pro_Process {
		...
		public function post_meta($fields, $user_id, $post_id) {
			...
			if (isset($_POST['usp-disable-alerts'])) $disable_alerts = add_post_meta($post_id, 'usp-disable-alerts', true);
		...
		public static function regex_filter($string, $args) {
			...
			$patterns[11] = "/%%post_id%%/";
			$patterns[12] = "/%%post_content%%/";
			$patterns[13] = "/%%post_custom%%/";
			$patterns[14] = "/%%post_submitted_date%%/";
			$patterns[15] = "/%%post_scheduled_date%%/";
			$patterns[16] = apply_filters('usp_alert_shortcut_defined_pattern', "/%%post_reject_reason%%/"); // ;)
			$patterns[17] = "/%%ip_address%%/";
			...
			$replacements[12] = $post_content;
			$replacements[13] = $custom_fields;
			$replacements[14] = $post_submitted; 
			$replacements[15] = $post_schedule;
			$replacements[16] = $post_defined;
			$replacements[17] = $ip_address;
			...
			if (isset($_POST['usp_form_submit']) && empty($_POST['usp-verify']) && wp_verify_nonce($_POST['usp_form_submit'], 'usp_form_submit')) {
				...
				if (isset($_POST['usp-captcha']) && !blank_or_zero($_POST['usp-captcha'])) {
					...
					if (isset($_POST['recaptcha_response_field'])) {
						...
						require_once(USP_PRO_PATH .'/lib/recaptchalib_v1.php');
						...
						$pass = ($recaptcha_response->is_valid) ? true : false;
						
					} elseif (isset($_POST['g-recaptcha-response'])) {
						
						require_once(USP_PRO_PATH .'/lib/recaptchalib_v2.php');
						$recaptcha_check = new ReCaptcha($recaptcha_secret);
						$recaptcha_response = $recaptcha_check->verifyResponse($_SERVER['REMOTE_ADDR'], $_POST['g-recaptcha-response']);
						$pass = ($recaptcha_response->success) ? true : false;
					}
					...
					foreach ($_POST as $key => $value) {
						...
						if ((preg_match("/^usp-custom-". $custom_field_regex ."$/i", $key, $match)) || (preg_match("/^$prefix". $custom_field_regex ."?$/i", $key, $match))) {
							...
							if (strpos($match[1], '-required') === false) {
								...
								if (is_array($value)) {
									foreach ($value as $val) {
										if (!blank_or_zero($val)) $usp_custom[$field . $match[1]][] = htmlspecialchars($val, ENT_QUOTES, get_option('blog_charset', 'UTF-8'));
									}
								} else {
									if (!blank_or_zero($value)) $usp_custom[$field . $match[1]] = htmlspecialchars($value, ENT_QUOTES, get_option('blog_charset', 'UTF-8'));
								}
					...
					foreach ($_POST as $key => $value) {
						...
						if (in_array($key, $custom_custom)) {
							if (preg_match("/^([0-9a-z_-]+)-required$/i", $key, $match)) {
								...
								if (is_array($value)) {
									foreach ($value as $val) {
										if (!blank_or_zero($val)) $usp_custom_custom[$key][] = htmlspecialchars($val, ENT_QUOTES, get_option('blog_charset', 'UTF-8'));
									}
								} else {
									if (!blank_or_zero($value)) $usp_custom_custom[$key] = htmlspecialchars($value, ENT_QUOTES, get_option('blog_charset', 'UTF-8'));
								}
							}
						}
					}
